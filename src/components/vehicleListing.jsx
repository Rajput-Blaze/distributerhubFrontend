...
export default class VehicleListing extends Component {
  ...
  getColorbyCarId(id) {
    service.getColorbyCarId(id).then((resp) => {
      if (resp?.data?.success) {
        this.setState({
                  ...
                  <div className='media media-side-list'>
                    <div className='media-sidebar'>
                      <div className='sidebar-style mr-50'>
                        ...
                        <div className='sidebar-widget'>
                          <h6 className='pro-sidebar-title'>
                            SELECT BY BUDGET
                          </h6>
                            ...
                            <form className='sidebar-widget-list' action='#'>
                              {this.state.brands
                                ...
                                .map((item, i) => (
                                  <div
                                    key={item._id}
                                    className='custom-control custom-radio cc-min-box pl-0'>
                                    <input
                                      type='radio'
                                      className='custom-control-input'
                                      id={`brands-${i}`}
                                      checked={this.state.brand.includes(
                                        item.brandName
                                      )}
                                    ...
                                    .map((item, i) => (
                                      <div
                                        ...
                                        <input
                                          ...
                                          className='custom-control-input'
                                          id={`customCheck-${i}`}
                                          checked={this.state.model.includes(
                                            item.vehicleDescription[0].plName
                                          )}
                                          value={
                                            item.vehicleDescription[0].plName
                                          }
                                          name='model'
                                          onChange={this.handleModelChange.bind(
                                            this,
                                            item.vehicleDescription[0]
                                              .vehicleName,
                                            item.VehicleName
                                          )}
                                        />
                              ...
                              <div className='pro-sidebar-search my-3'>
                                <form
                                  ...
                                  {this.state.colors.map((item, i) => (
                                    <div
                                      key={item._id}
                                      className='custom-control custom-checkbox cc-min-box pl-0'>
                                      <input
                                        type='checkbox'
                                        className='custom-control-input'
                                        id={`colors-${i}`}
                                        checked={this.state.color.includes(
                                          item.colorName
                                        )}
                            ...
                            <form className='sidebar-widget-list' action='#'>
                              ...
                              <div className='custom-control custom-checkbox cc-min-box pl-0'>
                                ...
                                <label
                                  className='custom-control-label'
                                  htmlFor='sizes-3'>
                                  CNG <span className='categories-num'></span>
                                </label>
                              </div>
                            </form>
                          </div>
                        </div>
                        <a className=' src-btn' href='#0' onClick={this.apply}>
                          Apply
                        </a>
                      </div>
                    </div>
                    <div className='media-body pl-0 pl-lg-4'>
                      ...
                      <div className='pagecontent-box'>
                        <h4>Best Cars</h4>
                        <p>
                          There are 15 cars available in India, among which
                          popular car models include Thar, Creta, Seltos,
                          Scorpio, Swift &amp; many more. The top Indian car
                          brands are.Hatchbacks and SUVs have dominated sales
                          figures in India. Here are some of the best-selling
                          cars in 2020: Maruti Swift,Maruti WagonR,Hyundai
                          Creta,Kia Seltos,Maruti Baleno.{' '}
                        </p>
                      </div>
                      <div className='shop-top-bar'>
                        <div className='select-shoing-wrap mt-3 mt-lg-0'>
                          <p>
                            Showing {this.state.vehicles.length} of{' '}
                        ...
                        {this.state.vehicles.map((item) => (
                          <div className='col-xl-3 col-lg-6 col-md-6 col-sm-6 col-12'>
                            <div className='x_car_offer_main_boxes_wrapper float_left'>
                              <Link
                                title={
                                  item.vehicleDescription[0].ppl[0].brands[0]
                                    .brandName +
                                  ' ' +
                                  item.vehicleDescription[0].vehicleName
                                }
                                className='title '
                                to={`vehicle-detail/${item.vehicleDescription[0]._id}/${item.vehicleDescription[0].ppl[0].pplName}/${item.variants[0].variantName}`}>
                                <div className='x_car_offer_img float_left'>
                                  {/* {JSON.stringify(item.variants[0])} */}
                                  <img
                                    src={getCarImageUrl(
                                      item.vehicleDescription[0].primaryImage
                                    )}
                                    alt='img'
                                  />
                                </div>
                                <div className='gsc_col-xs-12 holder truncate'>
                                  <span
                                    onClick={this.handleClickLink.bind(
                                      this,
                                      item.variants[0].selectVehicleType
                                        ? item.variants[0].selectVehicleType
                                        : ''
                                    )}>
                                    <Link
                                      title={
                                        item.vehicleDescription[0].ppl[0]
                                          .brands[0].brandName +
                                        ' ' +
                                        item.vehicleDescription[0].vehicleName
                                      }
